...
public class UserServiceTests {
	...
	public void acreateAdminUser() {
		...
		ResponseEntity<UserDTO> response = restTemplate.exchange(
				createURLWithPort("/guest/addUser"), HttpMethod.POST, entity,
				UserDTO.class);
		...
		ResponseEntity<LoginResponse> signinResponse = restTemplate.exchange(
				createURLWithPort("/api/auth/signin"), HttpMethod.POST,
				loginEntity, LoginResponse.class);
	...
	public void changePassword() {
		...
		HttpEntity<ResetPasswordDTO> resetPasswordEntity = new HttpEntity<ResetPasswordDTO>(
				passwordDTO, headers);
		ResponseEntity<Object> resetPasswordResponse = restTemplate.exchange(createURLWithPort("/user/changePassword"), HttpMethod.POST,
				resetPasswordEntity, Object.class);
		assertEquals(response.getStatusCode(), HttpStatus.CREATED);
		...
		HttpEntity<LoginRequest> loginEntityAgain = new HttpEntity<LoginRequest>(
				loginRequestAgain, headers);
		ResponseEntity<LoginResponse> signinResponseagain = restTemplate.exchange(
				createURLWithPort("/api/auth/signin"), HttpMethod.POST,
				loginEntityAgain, LoginResponse.class);
	...
	public void getMyProfile() {
		...
		headers.setContentType(MediaType.APPLICATION_JSON);
		headers.add("Authorization", this.userLoginResponse.getTokenType()
				+ " " + this.userLoginResponse.getAccessToken());

		HttpEntity<UserDTO> entity = new HttpEntity<UserDTO>(userDTO, headers);
		ResponseEntity<UserDTO> response = restTemplate.exchange(
				createURLWithPort("/user/getMyProfile"), HttpMethod.POST,
	...
	public void updateMyProfile() {
		...
		HttpEntity<UserDTO> entity1 = new HttpEntity<UserDTO>(
				userDTOFromResponse, headers);
		ResponseEntity<Boolean> response1 = restTemplate.exchange(
				createURLWithPort("/user/updateMyProfile"), HttpMethod.POST,
